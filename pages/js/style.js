(() => {
  const CFG = {
    grid: 48, speed: 0.08, amp: 10,
    shimmerSpeed: 0.004,
    particleCount: 140, particleSpeed: 0.08,
    parallax: 18, dprCap: 2
  };
  ...
  let w = 0, h = 0, dpr = Math.min(window.devicePixelRatio || 1, CFG.dprCap);
  function resize(){
  ...
  function getVars(){
    ...
    return {
      thread: cs.getPropertyValue('--thread').trim(),
      thread2: cs.getPropertyValue('--thread2').trim(),
      spark: cs.getPropertyValue('--spark').trim(),
    };
  }
  ...
  for(let i=0;i<CFG.particleCount;i++){
    const p={}; resetParticle(p); parts.push(p);
  }
  ...
  function draw(){
    ...
    tx += (mx*CFG.parallax - tx)*.04;
    ty += (my*CFG.parallax - ty)*.04;
    t+=CFG.speed;
    const g=CFG.grid;
    ...
    for(let y=-g;y<=h+g;y+=g){
      const phase=(y/h)*Math.PI*2 + t*.8;
      ctx.strokeStyle=vars.thread;
      ctx.beginPath();
      for(let x=-g;x<=w+g;x+=8){
        const yy=y+Math.sin(x*.03+phase)*CFG.amp+ty;
    ...
    for(let x=-g;x<=w+g;x+=g){
      const phase=(x/w)*Math.PI*2 + t*.9;
      ctx.strokeStyle=vars.thread2;
      ctx.beginPath();
      for(let y=-g;y<=h+g;y+=8){
        const xx=x+Math.cos(y*.03+phase)*CFG.amp+tx;
    ...
    grad.addColorStop(.5,`rgba(255,255,255,${a})`);
    ...
    for(const s of parts){
      ...
      ctx.globalAlpha=s.a;
      ctx.beginPath();
      ctx.arc(s.x+tx*.2,s.y+ty*.2,s.r,0,Math.PI*2);
